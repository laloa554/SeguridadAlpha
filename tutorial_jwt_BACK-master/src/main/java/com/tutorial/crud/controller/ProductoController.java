    ...
    public ResponseEntity<?> addUsers(/*@RequestBody String id*/){
    	//if(!id.isEmpty()) {
    		...
    		String body = "{\n"
    				+ "   \"Token\":\"77D5BDD4-1FEE-4A47-86A0-1E7D19EE1C74\"\n"
    				+ "}";
        	String j = e.conectaApiClubPOST(body, "http://192.168.20.26/ServiciosClubAlpha/api/Usuarios/GetUsuarios");
        	//JSONObject json = new JSONObject(j);
        	//System.out.println(j);
        	JSONArray a = new JSONArray(j);
        	String idCliente, membresia,email,x[];
        	for(Object ob: a) {
        		JSONObject item = (JSONObject) ob;
        		idCliente = String.valueOf(item.get("IDCliente"));
            	membresia = String.valueOf(item.get("Membresia"));
            	String getClienteById = e.conectaApiClubGET("http://192.168.20.26/ServiciosClubAlpha/api/Miembro/"+idCliente);
            	...
            	Usuario usuario = new Usuario(idCliente, membresia, email,passwordEncoder.encode(idCliente +"."+ x[0] + x[1] + x[2]),true);
            	Set<Rol> roles = new HashSet<>();
            	roles.add(rolService.getByRolNombre(RolNombre.ROLE_USER).get());
            	usuario.setRoles(roles);
            	usuarioService.save(usuario);
            	System.out.println(idCliente + "agregado");
    ...
    public ResponseEntity<?> desActivarUsuario(@RequestBody String id){
    	if(!id.isEmpty()) {
        	...
        	if(usuarioService.existsByNombreUsuario(idCliente) || usuarioService.existsByEmail(email)) {
        		...
        		if(Ux.get().isActivo()) {
        			Ux.get().setActivo(false);
        		}else {
        			Ux.get().setActivo(true);
        		}
