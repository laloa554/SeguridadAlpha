/*En nuestra clase PedidoDetalleDAOImpl, implementamos la interfaz, le añadiremos la anotación 
...
@Component("pedidoDetalleDAOImpl") //Es el estereotipo principal, indica que la clase anotada es un component.
...
public class PedidoDetalleDAOImpl implements PedidoDetalleDAO
{
	...
	//Método que inicia sesión y le asigna un query y a su vez se lo guarda a su lista, todo es de PEDIDODETALLE 
	public List<PedidoDetalle> findAll() 
	{
	     Session currentSession = entityManager.unwrap(Session.class);
	     Query<PedidoDetalle> theQuery = currentSession.createQuery("from PedidoDetalle", PedidoDetalle.class);
	     List<PedidoDetalle> pedidoDetalle = theQuery.getResultList();
	...
	{
		Session currentSession = entityManager.unwrap(Session.class);
        PedidoDetalle pedidoDetalle = currentSession.get(PedidoDetalle.class, id);
        return pedidoDetalle;
	}
	//Este último método guarda la informacion de PEDIDODETALLE
	@Override
    @Transactional
	public void save(PedidoDetalle pedidoDetalle) 
	{
		Session currentSession = entityManager.unwrap(Session.class);
        currentSession.saveOrUpdate(pedidoDetalle);  
	}
}
