/*En nuestra clase PagoDAOImpl, implementamos la interfaz, le añadiremos la anotación 
...
@Component("pagoDAOImpl") //Es el estereotipo principal, indica que la clase anotada es un component.
...
public class PagoDAOImpl implements PagoDAO
{
	...
	//Método que inicia sesión y le asigna un query y a su vez se lo guarda a su lista, todo es de PAGO.
	public List<Pago> findAll() 
	{
	     Session currentSession = entityManager.unwrap(Session.class);
	     Query<Pago> theQuery = currentSession.createQuery("from Pago", Pago.class);
	     List<Pago> pago = theQuery.getResultList();
	     return pago;
	}
	...
	{
		Session currentSession = entityManager.unwrap(Session.class);
        Pago pago = currentSession.get(Pago.class, id);
        return pago;
	}
	//Este último método guarda la informacion de PAGO
	@Override
    @Transactional
	public void save(Pago pago) 
	{
		Session currentSession = entityManager.unwrap(Session.class);
        currentSession.saveOrUpdate(pago);  
	}
}
