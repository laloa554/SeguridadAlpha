/*En nuestra clase TipoClienteDAOImpl, implementamos la interfaz, le añadiremos la anotación 
...
@Component("tipoClienteDAOImpl") //Es el estereotipo principal, indica que la clase anotada es un component.
...
public class TipoclienteDAOImpl implements TipoClienteDAO {
	...
	//Método que inicia sesión y le asigna un query y a su vez se lo guarda a su lista, todo es de TIPOCLIENTE
	public List<TipoCliente> findAll() {
		 Session currentSession = entityManager.unwrap(Session.class);

	        Query<TipoCliente> theQuery = currentSession.createQuery("from TipoCliente", TipoCliente.class);
	...
	public void save(TipoCliente tipoCliente) {
        ...
        currentSession.saveOrUpdate(tipoCliente);  
	}
	//Este último método guarda la informacion de TIPOCLIENTE
	@Override
	@Transactional
	public TipoCliente findById(int clave) {
		Session currentSession = entityManager.unwrap(Session.class);

		TipoCliente tipoCliente = currentSession.get(TipoCliente.class, clave);
