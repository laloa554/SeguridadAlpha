/*En nuestra clase MiercolesDAOImpl, implementamos la interfaz, le añadiremos la anotación 
...
@Component("miercolesDAOImpl") //Es el estereotipo principal, indica que la clase anotada es un component.
@Repository //Es un marcador para cualquier clase que cumpla el rol o el estereotipo de un repositorio (DAO). 
public class MiercolesDAOImpl implements MiercolesDAO {
	...
	//Método que inicia sesión y le asigna un query y a su vez se lo guarda a su lista, todo es de MIERCOLES. 
	public List<Miercoles> findAll() {
		 Session currentSession = entityManager.unwrap(Session.class);

	        Query<Miercoles> theQuery = currentSession.createQuery("from Miercoles", Miercoles.class);

	        List<Miercoles> miercoles = theQuery.getResultList();
	...
	public void save(Miercoles miercoles) {
        ...
        currentSession.saveOrUpdate(miercoles);  
	}
	//Este último método guarda la informacion de MIERCOLES
	@Override
	@Transactional
	public Miercoles findById(int clave) {
		Session currentSession = entityManager.unwrap(Session.class);

        Miercoles miercoles = currentSession.get(Miercoles.class, clave);
