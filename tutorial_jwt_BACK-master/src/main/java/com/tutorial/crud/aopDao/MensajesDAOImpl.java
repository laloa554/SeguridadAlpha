/*En nuestra clase MensajesDAOImpl, implementamos la interfaz, le añadiremos la anotación 
...
@Component("mensajesDAOImpl") //Es el estereotipo principal, indica que la clase anotada es un component.
...
public class MensajesDAOImpl implements MensajesDAO {
	...
	//Método que inicia sesión y le asigna un query y a su vez se lo guarda a su lista, todo es de MENSAJES. 
	public List<Mensajes> findAll() {
		 Session currentSession = entityManager.unwrap(Session.class);

	        Query<Mensajes> theQuery = currentSession.createQuery("from Mensajes", Mensajes.class);

	        List<Mensajes> mensajes = theQuery.getResultList();
	...
	public void save(Mensajes mensajes) {
        ...
        currentSession.merge(mensajes);  
	}
	//Este último método guarda la informacion de MENSAJES
	@Override
	@Transactional
	public Mensajes findById(int clave) {
		Session currentSession = entityManager.unwrap(Session.class);

        Mensajes mensajes = currentSession.get(Mensajes.class, clave);
