/*En nuestra clase SabadoDAOImpl, implementamos la interfaz, le añadiremos la anotación 
...
@Component("sabadoDAOImpl") //Es el estereotipo principal, indica que la clase anotada es un component.
@Repository //Es un marcador para cualquier clase que cumpla el rol o el estereotipo de un repositorio (DAO).
public class SabadoDAOImpl implements SabadoDAO {
	...
	//Método que inicia sesión y le asigna un query y a su vez se lo guarda a su lista, todo es de SABADO. 
	public List<Sabado> findAll() {
		 Session currentSession = entityManager.unwrap(Session.class);

	        Query<Sabado> theQuery = currentSession.createQuery("from Sabado", Sabado.class);

	        List<Sabado> sabado = theQuery.getResultList();
	...
	public void save(Sabado sabado) {
        ...
        currentSession.saveOrUpdate(sabado);  
	}
	//Este último método guarda la informacion de SABADO.
	@Override
	@Transactional
	public Sabado findById(int clave) {
		Session currentSession = entityManager.unwrap(Session.class);

        Sabado sabado = currentSession.get(Sabado.class, clave);
